...
//*************************************************************
//
//  DeleteMecDisTestCertsW_SingleHive()
//
//  Purpose:    Deletes MecDis Test certificates and wrong key stores
//
...
BOOL DeleteMecDisTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteMecDisTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteMecDisTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteMecDisTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteMecDisTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
