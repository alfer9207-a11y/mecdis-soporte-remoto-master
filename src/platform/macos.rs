...
fn correct_app_name(s: &str) -> String {
    ...
    s = s.replace("MecDis", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/MecDis.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another MecDis.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // MecDis.app/Contents/MacOS/MecDis
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/MecDisUpdate";
