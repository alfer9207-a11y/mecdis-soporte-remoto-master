...
pub fn is_rustdesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("MecDis Soporte Remoto")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/MecDis.app/Contents/MacOS/MecDis", "XPC_SERVICE_NAME=com.carriez.MecDis_server"
...
pub type MecDisInterval = ThrottledInterval;
...
mod tests {
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_MecDis_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
