...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
            if s.contains("MecDis")
                && !name.starts_with("upgrade_rustdesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("MecDis") {
                    s = s.replace("MecDis", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "MecDis" (e.g., "MecDis-Admin"), we need to avoid
                    // replacing "MecDis" within the already-substituted app_name, which would
                    // cause duplication like "MecDis-Admin" -> "MecDis-Admin-Admin".
                    //
