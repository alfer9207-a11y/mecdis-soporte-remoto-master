...
{
  HINSTANCE hInstance = LoadLibraryA("libmecdis.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libmecdis.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN mecdis_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "mecdis_core_main_args");
  if (!mecdis_core_main)
  {
    std::cout << "Failed to get mecdis_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = mecdis_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "MecDis Soporte Remoto [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"MecDis Soporte Remoto";
  FUNC_RUSTDESK_GET_APP_NAME get_mecdis_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_mecdis_app_name");
  if (get_mecdis_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_mecdis_app_name(app_name_buffer, 512) == 0) {
